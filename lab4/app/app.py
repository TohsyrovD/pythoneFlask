...
# загрузка данных из базы и инициализирует текущего пользователя
        ...
        if login and password:
            ...
            if db_user:
                user = User(user_id=db_user.id, login=db_user.login)
                 #класдет зашифрованный в куки 
                login_user(user, remember=remember_me)
                #   сообщение сохраняется в сессию
...
def logout():
    logout_user()
     # logout_user() - удаляет данные текущего пользователя из куки файла
...
def create():
    ...
    # Если мы не просто читаем, но и вносим изменения в базу данных - необходимо сохранить транзакцию2
